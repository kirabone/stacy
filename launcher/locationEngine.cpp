...
void FindApplication(const std::wstring& targetName) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    while (pEnum->Next(1, &pidlItem, &fetched) == S_OK && !found) {
                        ...
                        if (SUCCEEDED(pAppsFolder->GetDisplayNameOf(
                            pidlItem,
                            SHGDN_NORMAL,
                            &strRetName))) {
                            ...
                            if (pszName &&
                                ContainsIgnoreCase(pszName, targetName)) {
                                ...
                                if (SUCCEEDED(pAppsFolder->GetDisplayNameOf(
                                    ...
                                    std::wcout << L"Launching: " << pszName << std::endl;
                    ...
                    if (!found) {
                        std::wcout << L"Application not found."
                                   << std::endl;
                    }
...
int wmain(int argc, wchar_t* argv[]) {
    if (argc < 2) {
        std::wcout << L"Usage: launchingEngine.exe <AppName>"
                   << std::endl;
        return 1;
    }
...
int main(int argc, char* argv[]) {
    int argc_w;
    wchar_t** argv_w = CommandLineToArgvW(
        GetCommandLineW(),
        &argc_w
    );
